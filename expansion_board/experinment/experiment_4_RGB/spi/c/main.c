...
#define CURRENT_COLORS_SIZE             4                           // 当前氛围灯的灯光颜色数量
...
unsigned char rgb_buff[24];                                         // RGB数据
...
/*****************************
 * @brief : 更新单次传输给ws2812的rgb数据
 * @param : r 红色值
 * @param : g 绿色值
 * @param : b 蓝色值
 * @return: 无返回值
 *****************************/
void update_rgbbuff(unsigned char r, unsigned char g, unsigned char b)
{
    int i = 0;
...
void *ws2812_thread()
{
    ...
    while(ws2812_thread_stop != 1)
    {
        ...
        for (int i = 0; i <= 100; i++) 
        {
            ...
            update_rgbbuff(r*brightness, g*brightness, b*brightness);

            pthread_mutex_lock(spi_ops->mutex);
            spi_ops->spi_write_nbyte_data(rgb_buff, sizeof(rgb_buff));
            pthread_mutex_unlock(spi_ops->mutex);
