/*
*
*   file: oled_basic.c
*   update: 2024-10-22
*   usage: 
*       make
*       sudo ./oled_basic
*
*/

#include <sys/ioctl.h>
#include <errno.h>
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <linux/i2c.h>
#include <linux/i2c-dev.h>
#include <i2c/smbus.h>
#include "i2cbusses.h"
#include <time.h>
#include <stdint.h>
#include <signal.h> 
#include "font.h"

#define SIZE    (16)

uint8_t dev_addr = 0x3c;
int i2c_bus = 3;
int file;
char filename[20];

/*****************************
 * @brief : oled写命令
 * @param : cmd 要写入的命令
 * @return: none
*****************************/
void oled_write_command(uint8_t cmd)
{
    i2c_smbus_write_byte_data(file, 0x00, cmd);
}

/*****************************
 * @brief : oled写数据
 * @param : data 要写入的数据
 * @return: none
*****************************/
void oled_write_data(uint8_t data)
{
    i2c_smbus_write_byte_data(file, 0x40, data);
}

/*****************************
 * @brief : 设置数据写入的起始行列
 * @param : x 列起始位置
 * @param : y 行起始位置
 * @return: none
*****************************/
void oled_set_pos(uint8_t x, uint8_t y)
{
    oled_write_command(0xb0+y);				    
	oled_write_command((x&0x0f));  				
	oled_write_command(((x&0xf0)>>4)|0x10);		
}

/*****************************
 * @brief : 显示单个字符
 * @param : x 列起始位置
 * @param : y 行起始位置
 * @param : achar 要显示的单个字符（size: 8*16）
 * @return: none
*****************************/
void oled_show_char(uint8_t x, uint8_t y, uint8_t achar)
{
    uint8_t c=0, i=0;
    c = achar - ' ';

    if(x > 127)
    {
        x = 0;
        y = y + 2;
    }

    if(SIZE == 16)
    {
        oled_set_pos(x, y);
        for(i=0; i<8; i++)
            oled_write_data(F8X16[c*16+i]);

        oled_set_pos(x, y+1);
        for(i=0; i<8; i++)
            oled_write_data(F8X16[c*16+i+8]);
    }
}

/*****************************
 * @brief : 显示一行字符串
 * @param : x 列起始位置
 * @param : y 行起始位置
 * @param : string 要显示的字符串
 * @return: none
*****************************/
void oled_show_string(uint8_t x, uint8_t y, const uint8_t *string)
{
    uint8_t n_char = 0;

    while (string[n_char] != '\0') 	
	{
		oled_show_char(x, y, string[n_char]); 	

		x += 8;					
		if(x >= 128)
        {
            x = 0;
            y += 2;
        } 	

		n_char++; 				
	}
}

/*****************************
 * @brief : 显示单个中文字符
 * @param : x 列起始位置
 * @param : y 行起始位置
 * @param : no 汉字编号（size: 16*16）
 * @return: none
*****************************/
void oled_show_chinese(uint8_t x, uint8_t y, uint8_t no)
{
    uint8_t step = no * 2;
    uint8_t i = 0;

    if(x > 127)
    {
        x = 0;
        y = y + 2;
    }

    if(SIZE == 16)
    {
        oled_set_pos(x, y);
        for(i=0; i<16; i++)
            oled_write_data(F16X16[step][i]);

        oled_set_pos(x, y+1);
        for(i=0; i<16; i++)
            oled_write_data(F16X16[step+1][i]);
    }
}

/*****************************
 * @brief : 在 OLED 上绘制线段（考虑页模式）
 * @param : x0, y0 起点坐标
 * @param : x1, y1 终点坐标
 * @return: none
*****************************/
void oled_draw_line(int x0, int y0, int x1, int y1)
{
    
    int dx = abs(x1 - x0);
    int dy = abs(y1 - y0);
    int sx = (x0 < x1) ? 1 : -1;  // 步长，确定x方向
    int sy = (y0 < y1) ? 1 : -1;  // 步长，确定y方向
    int err = dx - dy;

    while (1) 
    {
        // 在(x0, y0)处绘制点
        int page = y0 / 8;
        uint8_t bit_mask = 1 << (y0 % 8);  // 计算具体 bit 位
        oled_set_pos(x0, page);
        oled_write_data(bit_mask);

        // 判断是否到达终点
        if (x0 == x1 && y0 == y1) break;

        // Bresenham算法更新坐标
        int e2 = err * 2;
        if (e2 > -dy) {
            err -= dy;
            x0 += sx;
        }
        if (e2 < dx) {
            err += dx;
            y0 += sy;
        }
    }

}

/*****************************
 * @brief : 在 OLED 上绘制斜线
 * @param : x0, y0 起点坐标
 * @param : x1, y1 终点坐标
 * @return: none
*****************************/
static void oled_draw_slant_line(int x0, int y0, int x1, int y1)
{
    // 确保起始和结束的顺序
    if (x0 > x1) {
        int tmp = x0;
        x0 = x1;
        x1 = tmp;
        int tmp_y = y0;
        y0 = y1;
        y1 = tmp_y;
    }

    int dx = x1 - x0; // x方向的变化
    int dy = y1 - y0; // y方向的变化
    int abs_dx = abs(dx); // x方向变化的绝对值
    int abs_dy = abs(dy); // y方向变化的绝对值

    int step = abs_dx > abs_dy ? abs_dx : abs_dy; // 最大变化步数
    float x_increment = (float)dx / step; // 每次x的增量
    float y_increment = (float)dy / step; // 每次y的增量

    float x = x0;
    float y = y0;

    // 绘制线段
    for (int i = 0; i <= step; i++) {
        int int_x = (int)(x + 0.5);  // 四舍五入
        int int_y = (int)(y + 0.5);  // 四舍五入

        // 设置当前点的页面和光标位置
        oled_set_pos(int_x, int_y / 8);  
        
        uint8_t bit_mask = 1 << (int_y % 8);  // 计算具体 bit 位
        oled_write_data(bit_mask);  // 设置当前点亮的位

        // 更新 x 和 y 坐标
        x += x_increment;  
        y += y_increment;  
    }
}

/*****************************
 * @brief : 在 OLED 上绘制垂直线
 * @param : x  列位置
 * @param : y0 起点行位置
 * @param : y1 终点行位置
 * @return: none
*****************************/
static void oled_draw_vertical_line(int x, int y0, int y1)
{
    if (y0 > y1) 
    {
        int tmp = y0;
        y0 = y1;
        y1 = tmp;
    }

    // 确定垂直线涉及的页范围
    int start_page = y0 / 8;
    int end_page = y1 / 8;

    // 绘制每个页中的部分
    for (int page = start_page; page <= end_page; page++) 
    {
        oled_set_pos(x, page);                  // 设置列地址和页地址

        // 计算当前页需要点亮的bit掩码
        uint8_t mask = 0xFF;                    // 默认全亮
        if (page == start_page) 
            mask &= (0xFF << (y0 % 8));         // 起始页掩码

        if (page == end_page) 
            mask &= (0xFF >> (7 - (y1 % 8)));   // 终止页掩码

        oled_write_data(mask);                  // 写入数据
    }
}

/*****************************
 * @brief : 在 OLED 上绘制矩形
 * @param : x0, y0 左上角坐标
 * @param : x1, y1 右下角坐标
 * @return: none
*****************************/
void oled_draw_rectangle(int x0, int y0, int x1, int y1)
{
    oled_draw_line(x0, y0, x1, y0);         // 上边
    oled_draw_line(x0, y1, x1, y1);         // 下边
    oled_draw_vertical_line(x0, y0, y1);    // 左边
    oled_draw_vertical_line(x1, y0, y1);    // 右边
}

/*****************************
 * @brief : 在 OLED 上绘制三角形
 * @param : x0, y0 顶点1坐标
 * @param : x1, y1 顶点2坐标
 * @param : x2, y2 顶点3坐标
 * @return: none
*****************************/
static void oled_draw_triangle(int x0, int y0, int x1, int y1, int x2, int y2)
{
    // 绘制三角形的三条边
    oled_draw_slant_line(x0, y0, x1, y1); // 从顶点到左底边
    oled_draw_slant_line(x0, y0, x2, y2); // 从顶点到右底边
    oled_draw_line(x1, y1, x2, y2);       // 底边
}

static void oled_draw_circle(int x0, int y0, int radius) 
{
    int x = radius;
    int y = 0;
    int decision = 1 - radius;  // 初始决策参数

    while (x >= y) {
        // 在四个象限绘制点
        oled_draw_line(x0 + x, y0 + y, x0 + x, y0 + y);  // 右上
        oled_draw_line(x0 - x, y0 + y, x0 - x, y0 + y);  // 左上
        oled_draw_line(x0 + x, y0 - y, x0 + x, y0 - y);  // 右下
        oled_draw_line(x0 - x, y0 - y, x0 - x, y0 - y);  // 左下
        oled_draw_line(x0 + y, y0 + x, x0 + y, y0 + x);  // 右下斜
        oled_draw_line(x0 - y, y0 + x, x0 - y, y0 + x);  // 左下斜
        oled_draw_line(x0 + y, y0 - x, x0 + y, y0 - x);  // 右上斜
        oled_draw_line(x0 - y, y0 - x, x0 - y, y0 - x);  // 左上斜

        // 画出圆周上的点
        oled_draw_line(x0 + x, y0 + y, x0 + x, y0 + y);  // (x, y)
        oled_draw_line(x0 - x, y0 + y, x0 - x, y0 + y);  // (-x, y)
        oled_draw_line(x0 + x, y0 - y, x0 + x, y0 - y);  // (x, -y)
        oled_draw_line(x0 - x, y0 - y, x0 - x, y0 - y);  // (-x, -y)
        oled_draw_line(x0 + y, y0 + x, x0 + y, y0 + x);  // (y, x)
        oled_draw_line(x0 - y, y0 + x, x0 - y, y0 + x);  // (-y, x)
        oled_draw_line(x0 + y, y0 - x, x0 + y, y0 - x);  // (y, -x)
        oled_draw_line(x0 - y, y0 - x, x0 - y, y0 - x);  // (-y, -x)

        y++;

        // 更新决策参数
        if (decision <= 0) {
            decision += 2 * y + 1;  // 中间点在圆内
        } else {
            x--;
            decision += 2 * y - 2 * x + 1;  // 中间点在圆外
        }
    }
}

/*****************************
 * @brief : oled清屏
 * @param : none
 * @return: none
*****************************/
void oled_clear(void)
{
    uint8_t page, row;

	for(page=0; page<8; page++)
	{
		oled_write_command(0xb0 + page);	    
		oled_write_command(0x00);      		
		oled_write_command(0x10);      		

		for(row=0; row<128; row++)
            oled_write_data(0x00);
	}
}

/*****************************
 * @brief : oled初始化
 * @param : none
 * @return: none
*****************************/
int oled_init(void)
{
    file = open_i2c_dev(i2c_bus, filename, sizeof(filename), 0);
	if (file < 0)
	{
		printf("can't open %s\n", filename);
		return -1;
	}

	if (set_slave_addr(file, dev_addr, 1))
	{
		printf("can't set_slave_addr\n");
		return -1;
	}

    oled_write_command(0xAE);    //显示打开

    oled_write_command(0x81);    //设置对比度
    oled_write_command(0xFF);

    oled_write_command(0xA4);    //使能全屏显示，恢复到RAM内容显示
    oled_write_command(0xA6);    //设置显示模式，正常显示：0灭1亮

    oled_write_command(0x2E);    //使能滚动
    oled_write_command(0x26);    //右水平滚动
    oled_write_command(0x00);    //虚拟字节
    oled_write_command(0x00);    //设置滚动起始页地址
    oled_write_command(0x03);    //设置滚动间隔
    oled_write_command(0x07);    //设置滚动结束地址

    oled_write_command(0x00);    //虚拟字节
    oled_write_command(0xFF);

    oled_write_command(0x20);    //寄存器寻址模式
    oled_write_command(0x10);    //页寻址模式
    oled_write_command(0xB0);    //设置页寻址的起始页地址（B0-B7:page0-page7）
    oled_write_command(0x00);    //设置页寻址的起始列地址低位
    oled_write_command(0x10);    //设置页寻址的起始列地址高位

    oled_write_command(0x40);    //设置显示开始线，0x40~0x7F对应0~63

    oled_write_command(0xA1);    //设置列重映射，addressX--->seg(127-X)

    oled_write_command(0xA8);    //设置多路复用比
    oled_write_command(0x3F);

    oled_write_command(0xC8);    //设置COM输出扫描方向，C8：COM63--->COM0(从下往上扫描)

    oled_write_command(0xD3);    //设置COM显示不偏移
    oled_write_command(0x00); 

    oled_write_command(0xDA);    //配置COM重映射
    oled_write_command(0x12);

    oled_write_command(0xD9);    //设置预充期
    oled_write_command(0x22); 

    oled_write_command(0xDB);    //设置VCOMH取消选择电平
    oled_write_command(0x20);

    oled_write_command(0x8d);    //设置电荷泵
    oled_write_command(0x14);

    oled_write_command(0xAF);

    oled_clear();                //清屏
}

void sigint_handler(int sig_num) 
{    
    oled_clear();
    exit(0);  
}

int main(int argc, char **argv)
{
    int ret;
    int start_x;
    char *str_title = "Lubancat";
    char *str_slogan = "hyw666";

    /* register exit signal ( Ctrl + c ) */
    signal(SIGINT, sigint_handler);

    /* oled init */
    ret = oled_init();
    if(ret == -1)
    {
        printf("oled init err!\n");
        return -1;
    }
    
    oled_draw_circle(64, 32, 20);  // 在 (64, 32) 绘制半径为 20 的圆


    while(1)
    {
        sleep(1);
    }

    return 0;
}